...
/mob/living/carbon/proc/print_thirst(var/mob/user)
	switch(thirst)
		if(THIRST_LEVEL_THIRSTY to INFINITY)
			to_chat(user, "<span class = 'notice'>I am feeling hydrated!</span>")
		if(THIRST_LEVEL_DEHYDRATED to THIRST_LEVEL_THIRSTY)
			to_chat(user, "I am feeling a little thirsty.")
		if(0 to THIRST_LEVEL_DEHYDRATED)
			to_chat(user, "<span class = 'warning'>[pick("I am parched!","I am feeling dehydrated!","So... Thirsty...")]</span>")

/mob/living/carbon/proc/print_hunger(var/mob/user)
	switch(nutrition)
		if(HUNGER_SATED to INFINITY)
			to_chat(user, "<span class = 'notice'>I am not feeling hungry!</span>")
		if(HUNGER_HUNGRY to HUNGER_SATED)
			to_chat(user, "I am feeling hungry.")
		if(HUNGER_STARVING to HUNGER_HUNGRY)
			to_chat(user, "<span class = 'warning'>I am starving!</span>")
		if(0 to HUNGER_STARVING)
			to_chat(user, "<span class = 'warning><b>I am wasting away!</b></span>")