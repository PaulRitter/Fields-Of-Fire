...
/mob/living/Move(a, b, flag)
	...
	for(var/obj/item/weapon/storage/stor in s_active)
		if (!( stor in contents ) && get_turf(stor) != get_turf(src))	//check !( s_active in contents ) first so we hopefully don't have to call get_turf() so much.
			stor.close(src)
